...
namespace kw {
	...
	uint16_t BuddySystemAllocator::allocate(size_t _size, size_t* offset_ ) {
	...
	bool BuddySystemAllocator::free(uint16_t _handle) {
		assert(_handle < m_nodeTable.size());
		size_t layer = 0;
		uint16_t v = _handle;
		while (v != 1) {
			layer++;
		...
		node_t& node = m_nodeTable[_handle];
		updateTableForFree(node, layer, _handle);
		return true;
	}
