...
namespace base64
{
	...
	{
		uint8_t* reinterpret_uint8_ptr = (uint8_t*)_base64;
		assert(_length >= 4);
		...
		if (_length > 4) {
			for (; i < _length - 4; i += 4) {
				*ptr = (DecTable[reinterpret_uint8_ptr[i]] << 2) | (DecTable[reinterpret_uint8_ptr[i + 1]] >> 4); ++ptr;
				*ptr = ((DecTable[reinterpret_uint8_ptr[i + 1]] & 0xf) << 4) | (DecTable[reinterpret_uint8_ptr[i + 2]] >> 2); ++ptr;
				*ptr = ((DecTable[reinterpret_uint8_ptr[i + 2]] & 0b11) << 6) | DecTable[reinterpret_uint8_ptr[i + 3]]; ++ptr;
			}
		}
		*ptr = (DecTable[reinterpret_uint8_ptr[i]] << 2) | (DecTable[reinterpret_uint8_ptr[i + 1]] >> 4); ++ptr;
		if (reinterpret_uint8_ptr[i + 2] == '=') {
			return ptr - _data;
		}			
		*ptr = ((DecTable[reinterpret_uint8_ptr[i + 1]] & 0xf) << 4) | (DecTable[reinterpret_uint8_ptr[i + 2]] >> 2); ++ptr;
		if (reinterpret_uint8_ptr[i + 3] == '=') {
			return ptr - _data;
		}			
		*ptr = ((DecTable[reinterpret_uint8_ptr[i + 2]] & 0b11) << 6) | DecTable[reinterpret_uint8_ptr[i + 3]]; ++ptr;
		return ptr - _data;
